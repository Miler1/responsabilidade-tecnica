...
FROM
	(SELECT id FROM portal_seguranca.usuario u) AS users,
	(SELECT id
	 FROM portal_seguranca.perfil p
	 WHERE codigo = 'USUARIO' AND id_modulo_pertencente = (SELECT id FROM portal_seguranca.modulo m WHERE sigla = 'GRT')) AS profiles
